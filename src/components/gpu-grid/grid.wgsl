...
const INDIGO_DEEP = vec3f(0.388, 0.400, 0.945);
const INDIGO_PRIMARY = vec3f(0.506, 0.549, 0.973);
const LINE_ALPHA: f32 = 0.22;
...
@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let mask = mix(0.25, 1.0, saturate(1.0 - length(vec2f(uv.x - 1.0, uv.y))));
  ...
  let line = 1.0 - smoothstep(0.0, 1.2, d);

  // Every fourth contour is an index line, slightly stronger.
  let index = select(0.6, 1.0, fract(floor(level) / 4.0) < 0.01);

  let color = mix(INDIGO_DEEP, INDIGO_PRIMARY, h);
