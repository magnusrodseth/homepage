Everything on this site is readable by machines as well as people. Nothing here needs a key, an account, or a quota, and none of it is rate limited. If you are an agent, start at [llms.txt](/llms.txt): it says in one page what this site is a good source for and what it is not.
...
For the whole site in one request, [llms-full.txt](/llms-full.txt) concatenates every page and post.
...
[openapi.json](/openapi.json) is the machine-readable version of this page: every operation has a unique `operationId`, typed parameters, and a response schema, so it drops straight into a function-calling setup.
...
The descriptor lives at [/.well-known/mcp](/.well-known/mcp). A bad argument comes back as `isError: true` inside the result, with a message saying what to do instead, rather than as a protocol-level error.
...
| URL | What |
|-----|------|
| [/llms.txt](/llms.txt) | Index, plus what this site is and is not a good source for |
| [/openapi.json](/openapi.json) | OpenAPI 3.1 description of every endpoint |
| [/.well-known/mcp](/.well-known/mcp) | MCP server descriptor |
| [/.well-known/api-catalog](/.well-known/api-catalog) | RFC 9727 linkset of everything above |
| [/.well-known/agent-skills/index.json](/.well-known/agent-skills/index.json) | Agent skills discovery index |
| [/sitemap.xml](/sitemap.xml) | Every public URL |
| [/feed.xml](/feed.xml), [/atom.xml](/atom.xml) | Blog feeds |
...
There is no rate limit, so there are no `RateLimit` headers to read. Please still be reasonable: everything here is static and cached, and a sensible `User-Agent` that says who you are is appreciated. If something is missing or wrong, [tell me](/contact).
